###This script calculates correlations for all regulons X all cancer types
...
#selects regulon list based on regulons chosen
if (regulons_types == "aracne") {
  regulons_list = c("aracne_aml", "aracne_blca", "aracne_brca", "aracne_coad", 
                    "aracne_gbm", "aracne_hnsc", "aracne_kirc", 
                    "aracne_luad", "aracne_paad", "aracne_stad")
  
} else if(regulons_types == "grndb") {
  regulons_list = c("grndb_aml", "grndb_blca", "grndb_brca", "grndb_coad", 
                    "grndb_gbm", "grndb_hnsc", "grndb_kirc", 
                    "grndb_luad", "grndb_paad", "grndb_stad")
...
for (cancer_label in cancer_list) { #loop1 for each cancer
  for (regulon in regulons_list){ #loop2 for each regulon
    ...
    non_essential_genes_per_cancer <- gene_effect_tmp_signif_reg_genes[apply(gene_effect_tmp_signif_reg_genes, 1, function(x) max(x, na.rm = TRUE) > -0.6), drop = FALSE, ] %>%
      rownames()
    
    #select genes that are essential in at least one cell line
    essential_genes_per_cancer <- gene_effect_tmp_signif_reg_genes[apply(gene_effect_tmp_signif_reg_genes, 1, function(x) min(x, na.rm = TRUE) < -0.6), drop = FALSE, ] %>%
      rownames()
    
    
    #keep only genes essential in at least one cell line
    gene_effect_tmp_signif_reg_genes <- 
      gene_effect_tmp_signif_reg_genes[(rownames(gene_effect_tmp_signif_reg_genes) %in% essential_genes_per_cancer), ]
    
    #keep only genes NON-essential in at least one cell line
    gene_effect_tmp_signif_reg_genes <- 
      gene_effect_tmp_signif_reg_genes[(rownames(gene_effect_tmp_signif_reg_genes) %in% non_essential_genes_per_cancer), ]
    
    
    gene_effect_tmp_signif_reg_genes <- as.data.frame(t(as.matrix(gene_effect_tmp_signif_reg_genes))) #transpose
    
    signif_genes_vector <- colnames(gene_effect_tmp_signif_reg_genes) #vector containing all genes for which correlations are calculated
    
    #empty dataframe to populate with correlations
    corr_per_gene_matrix <- data.frame(matrix(, nrow = length(signif_genes_vector), ncol = 13))
    rownames(corr_per_gene_matrix) <- signif_genes_vector #all genes as rownames
    ...
    print(paste0("computing correlations ", regulon, "+", cancer_label))
    
    #fill matrix with correlations results between activity and gene effect
    gene_counter <- 0 #having a gene counter to track progress when running the script
    for (gene in signif_genes_vector) {
      ...
      corr_per_gene_matrix[gene, "hgnc_symbol"] <- gene #setting the gene also here 
      ...
      print(paste("DONE activity/essentiality corr Gene", gene_counter, "out of", length(signif_genes_vector)))
    }
    rm(gene)  
    
    
    #create empty matrix for expression
    corr_per_gene_exp_matrix <- data.frame(matrix(, nrow = length(rownames(t_gene_exp_tmp_subset)), ncol = 3))
    rownames(corr_per_gene_exp_matrix) <- rownames(t_gene_exp_tmp_subset)
...
output_dir <- paste0("./results/correlation_matrices/", regulons_types)
